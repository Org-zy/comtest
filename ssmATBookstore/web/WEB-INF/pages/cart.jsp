...
<script type="text/javascript">
	...
	$(function(){
		...
		$(":text").change(function(){
			...
			var reg = /^\d+$/g;//验证是否是数字的正则表达式（js或这jquery中）
			var quantity = -1;
			if(reg.test(quantityVal)){//验证输入框的值是否是数字是为ture
				quantity = parseInt(quantityVal);//将输入框的值转换为整型
				if(quantity >= 0){
					flag = true;
				}
			}
			...
			if(!flag){
				alert("输入的数量不合法!");
				$(this).val($(this).attr("class"));//通过class的值保存原来的输入的值，并且将他的值设置为输入框的值
				return;
			}
			...
			$.post(url, args, function(data){//ajax请求$.post{url,参数，出理方法，"json"}
